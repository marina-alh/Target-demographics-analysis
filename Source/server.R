...
shinyServer(function(input, output) {
  ...
  output$p1 <- renderPlot({
    if (input$graph_type == "histogram") {
      ...
      ggplot(df_country(), aes_string(y = input$continuous_variable)) +
        ...
        labs(title = paste("Distribution of ", input$continuous_variable)) +  # labels
        facet_grid(~ prediction)   # facet by prediction
    }
  ...
  output$p2 <- renderPlot({
    ...
    p <- ggplot(df_country(), aes_string(x = input$categorical_variable)) +
         labs(title = paste("Distribution of ", input$categorical_variable)) +  # labels
         theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=1))    # modify theme to change text angle and legend position
